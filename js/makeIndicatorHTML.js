...
const makeIndicatorHTML = async (params) => {
  ...
  // create content
  const headerFrag = makeHeader(title, categories, params.formattedTitle);
  const { descriptionFrag, description } = {
    ...makeDescription(defaultText, trend),
  };
...
const make404 = () => {
  ...
  text.innerHTML =
    'Sorry! The requested indicator page was not found. Please <a href="/TrackingProgress/">click here</a> to navigate back to the dashboard.';
...
// START header
const makeHeader = (title, categories, formattedTitle) => {
  // make elements
  ...
  // add content
  header.textContent = formattedTitle ?? title;
