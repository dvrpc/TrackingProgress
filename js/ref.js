...
const snippetsRef = {
    ...
    'Exports': {
        ...
        text: {
            ...
            what: `<p>Initial declarations of merchandise export values are reported at the metropolitan and county level by the International Trade Administration using data from the US Census Bureau's Origin of Movement–ZIP code Based Series (OM-ZIP). This represents the exports of goods. Services exports are not included in this dataset.</p><p>Goods exports data reported at the sub-national level rely on initial declarations and do not get updated with subsequent revisions found in national data. Export county is assigned using the address of the United States Principal Party in Interest (USPPI) of record—which is often, but not always the entity producing the merchandise. A small portion of the overall metropolitan Core Based Statistical Area (CBSA) export value does not have a county determination. For this analysis, DVRPC assigned this residual CBSA value to counties according to the portion of the CBSA’s export value attributable to each county. Despite inherent errors, these data are a relevant gauge of where the region stands based off previous years. Merchandise export data is available starting in 2012.</p><p><a href="https://www.trade.gov/mas/ian/metroreport/tg_ian_002825.asp" target="blank">Link</a> to International Trade Administration data methodology.</p>`,
